...
def cumsum_reset(np_arr):
    ...
    return np.array(
        pd.DataFrame(np_arr).astype(int).apply(lambda x: x.groupby((~x.astype(bool)).cumsum()).cumsum())).reshape(-1)
        ...
        for index in range(X_train.shape[1]):
            ...
            if plot:
                ...
                for lab in labs:
                    ...
                    axes[0][0].hist(data_test[y_test == lab], np.linspace(min, max, n_points),
                                    color=cts.COLORS[lab + 2],
