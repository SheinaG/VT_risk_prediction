...
def comp_sampEn(y, M, r):
    ...
    A = np.array([(np.sum(np.max(np.abs(repeated_X_A[i] - tiled_X_A[i]), axis=1) < r) - len_X_A[i]) / 2 for i in
                  range(M)]).reshape(M, 1)
    B = np.array([(np.sum(np.max(np.abs(repeated_X_B[i] - tiled_X_B[i]), axis=1) < r) - len_X_B[i]) / 2 for i in
                  range(M)]).reshape(M, 1)
    N = n * (n - 1) / 2
    p[0] = A[0] / N
    e[0] = -np.log(p[0])
...
if __name__ == '__main__':
    ...
    for i, rr in enumerate(rr_intervals):
        ...
        features_python = [cosEn, AFEv, OriginCount, IrrEv, PACEv, AVNN, SDNN, SEM, minRR, medHR, PNN20, PNN50, RMSSD,
                           CV, SD1, SD2, PIP, IALS, PSS, PAS]
    ...
    F1_matlab, IndMatch_matlab, meanDist_matlab = results_matlab['F1'], results_matlab['IndMatch'], results_matlab[
        'meanDist']
