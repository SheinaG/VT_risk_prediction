...
class ResNet(nn.Module):
    ...
    def make_layers(self, num_layers, block, num_residual_blocks, intermediate_channels, stride):
        ...
        identity_downsample = nn.Sequential(
            nn.Conv1d(self.in_channels, intermediate_channels * self.expansion, kernel_size=1, stride=stride),
...
def ff_test():
    net = ResNet18(img_channels=1, num_classes=2)
    y = net(torch.randn(64, 1, 2000 * 180)).to("cuda")
    print(y.size())
...
if __name__ == '__main__':
    # count_parameters(ResNet18(img_channels=1, num_classes=2))
    ff_test()
