...
def totensor(o):
    if isinstance(o, torch.Tensor):
        ...
        return torch.from_numpy(o.values)
    else:
        try:
            return torch.tensor(o)
        except:
            warn(f"Can't convert {type(o)} to torch.Tensor", Warning)
...
def toarray(o):
    if isinstance(o, np.ndarray):
        ...
        try:
            ...
            warn(f"Can't convert {type(o)} to np.array", Warning)


def toL(o):
    if isinstance(o, L):
        return o
    elif isinstance(o, (np.ndarray, torch.Tensor)):
        return L(o.tolist())
    else:
        try:
            return L(o)
        except:
            warn(f'passed object needs to be of type L, list, np.ndarray or torch.Tensor but is {type(o)}', Warning)
...
def get_size(
    ...
    s = sys.getsizeof(o)
    if return_str:
        return bytes2size(s, decimals=decimals)
    else:
        return s
...
def get_func_defaults(f):
    ...
    if fa.defaults is None:
        return dict(zip(fa.args, [''] * (len(fa.args))))
    else:
        return dict(zip(fa.args, [''] * (len(fa.args) - len(fa.defaults)) + list(fa.defaults)))
...
# Cell
def create_array(shape, fname=None, path='./data', on_disk=True, dtype='float32', mode='r+', fill_value='rand',
...
def rolling_moving_average(o, window=2):
    if isinstance(o, torch.Tensor):
        ...
        lag_cunsum = torch.cat(
            [torch.zeros((o.shape[0], o.shape[1], window), device=o.device), torch.cumsum(o[..., :-window], axis=-1)],
            -1)
        count = torch.clip(torch.ones_like(o).cumsum(-1), max=window)
        return (cunsum - lag_cunsum) / count
    else:
        cunsum = np.nancumsum(o, axis=-1)
        lag_cunsum = np.concatenate(
            [np.zeros((o.shape[0], o.shape[1], window)), np.nancumsum(o[..., :-window], axis=-1)], -1)
